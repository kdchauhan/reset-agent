...
export default function ResetAgentUI() {
  ...
  const [hoursFasted, setHoursFasted] = useState(0);
  const [slipLogged, setSlipLogged] = useState(false);
  ...
  useEffect(() => {
    if (startTime) {
      const interval = setInterval(() => {
        const now = new Date();
        const diff = (now - new Date(startTime)) / 36e5;
        setHoursFasted(diff.toFixed(2));
      }, 60000);
  ...
  const milestoneText = () => {
    const hours = parseFloat(hoursFasted);
    if (hours < 12) return 'Glycogen is being used.';
    if (hours < 24) return 'Transitioning into fat burn.';
    if (hours < 48) return 'In ketosis. Growth hormone rising.';
    if (hours < 72) return 'Deep fat burn and cellular cleanup.';
    return 'Autophagy in full effect. Mental clarity likely peaking.';
  };

  return (
    <div style={{ fontFamily: 'Arial, sans-serif', padding: '2rem', maxWidth: '600px', margin: 'auto' }}>
      <h1>Reset Agent UI</h1>
